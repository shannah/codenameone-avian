...
- (id)initWithURL:(NSString*)url callback:(void*)callback {
    ...
    if (self) {
        ...
        if ( runnableCallback != NULL ){
            runnableCallbackClass = (*env)->GetObjectClass(env, callback);
            runnableCallbackMethodId = (*env)->GetMethodID(env, runnableCallbackClass, "run", "()V");
        }
        errorInfo = nil;
...
- (void)audioPlayerDecodeErrorDidOccur:(AVAudioPlayer *)player error:(NSError *)error {
    if(runnableCallback != 0) {
        JNIEnv *env = [CodenameOne_GLAppDelegate jniEnv];
        (*env)->CallVoidMethod(env, runnableCallback, runnableCallbackMethodId);
    }
}
